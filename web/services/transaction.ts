...
import type { ListTransactionsInput, SyncTransactionInput } from "@/schemas/transaction";
...
export async function listTransactions(userId: string, query: ListTransactionsInput) {
  ...
  if (query.startDate || query.endDate) {
    filter.timestamp = {};
    if (query.startDate) (filter.timestamp as Record<string, Date>).$gte = new Date(query.startDate);
...
export async function syncTransactions(
  userId: string,
  data: SyncTransactionInput
) {
  await connectDB();
  const results: Array<{ localId: string; status: string; serverId?: string; error?: string }> = [];
      ...
      if (item.deletedAt) {
        ...
        if (existing) {
          ...
          if (item.version >= existing.version) {
            ...
            results.push({ localId: item.localId, status: "updated", serverId: existing._id.toString() });
          } else {
            results.push({ localId: item.localId, status: "conflict", error: "Server has newer version" });
          }
        } else {
          ...
          results.push({ localId: item.localId, status: "created", serverId: created._id.toString() });
        }
      }
    } catch (err) {
      logger.error({ err, localId: item.localId }, "Sync error for transaction");
      results.push({ localId: item.localId, status: "error", error: (err as Error).message });
    }
  }
